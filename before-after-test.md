...
export default function StacksPage() {
  ...
  async function handleCreateStack() {
    ...
    const { data: newHabit } = await supabase
      ...
      .insert({ name: newHabitName, user_id: 'current-user-id', type: 'positive' })
      .select()
      .single();
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-8">
      <h1 className="text-3xl font-bold mb-6">Habit Stacks</h1>
      ...
      <div className="space-y-4">
        {stacks.map((stack) => (
          <div key={stack.id} className="bg-gray-100 rounded-lg p-4 flex items-center gap-4">
            <div className="bg-white rounded p-3 shadow-sm flex-1">
              {/* Problem: No tag color coding (green/red/gray) */}
              <p className="font-medium">
                {habits.find((h: any) => h.id === stack.anchor_habit)?.name || 'Unknown'}
              </p>
            </div>
            ...
            <div className="bg-white rounded p-3 shadow-sm flex-1">
              <p className="font-medium">
                {habits.find((h: any) => h.id === stack.new_habit)?.name || 'Unknown'}
              </p>
            </div>
...
export function useStacks(initialStacks: HabitStackWithRoutines[]) {
  ...
  const createStack = useCallback(async (input: CreateStackInput) => {
    ...
    try {
      ...
      if (data) setStacks((prev) => [...prev, data as HabitStackWithRoutines]);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to create stack';
      setError(message);
  ...
  const deleteStack = useCallback(async (stackId: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to delete stack';
      setError(message);
    }
  }, [supabase]);
...
export default async function StacksPage() {
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:max-w-3xl lg:mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Habit Stacks</h1>
...
export function StackBuilder({ routines }: StackBuilderProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
      <h2 className="text-lg font-semibold text-gray-900 mb-4">Create a Stack</h2>
      ...
      <div className="space-y-3">
        <div>
          <label htmlFor="anchor" className="block text-sm font-medium text-gray-500 mb-1">
            After I...
          </label>
          <select
            id="anchor"
            value={anchorId}
            onChange={(e) => setAnchorId(e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-gray-900
        ...
        <div>
          <label htmlFor="stacked" className="block text-sm font-medium text-gray-500 mb-1">
            I will...
          </label>
          <select
            id="stacked"
        ...
        <button
          ...
          className="w-full bg-indigo-500 text-white font-medium py-2.5 rounded-lg
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span
          className={`w-2.5 h-2.5 rounded-full shrink-0 ${TAG_COLORS[routine.tag]}`}
          aria-label={`${routine.tag} habit`}
        />
        <span className="text-sm font-medium text-gray-900">{routine.name}</span>
        {isCompleted && (
          <span className="ml-auto text-green-500 text-sm" aria-label="Completed">✓</span>
        )}
      </div>
    </div>
  );
      ...
      {stacks.map((stack) => (
        <div key={stack.id} className="flex flex-col items-center gap-0">
          ...
          <div className="flex flex-col items-center py-1">
            ...
            <span className="text-xs text-gray-400 font-medium py-0.5">then</span>
            <div className="w-px h-4 bg-gray-300" />
          </div>
...
const mockRoutines: Routine[] = [
  {
    id: '1', user_id: 'u1', name: 'Morning coffee',
    tag: 'neutral', time_of_day: 'morning', sort_order: 0,
    archived_at: null, created_at: '', updated_at: '',
  },
  {
    id: '2', user_id: 'u1', name: 'Read 10 pages',
    tag: 'positive', time_of_day: 'morning', sort_order: 1,
    archived_at: null, created_at: '', updated_at: '',
  },
];
...
describe('StackBuilder', () => {
  ...
  it('disables create button when no selections are made', () => {
    render(<StackBuilder routines={mockRoutines} />);
    expect(screen.getByRole('button', { name: /create stack/i })).toBeDisabled();
  });
